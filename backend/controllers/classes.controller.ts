...
const signNewClass = async (req: express.Request, res: express.Response) => {
  ...
  if(await checkIfDateIsValid(idInstructor,id,date,hours,minutes,longHour) == false){
...
const getAllClassesEndPoint = async (req:express.Request,res:express.Response) =>{
...
const getAllClassesUser = async (req:express.Request,res:express.Response) =>{
  let params = parseQueryParams(req.originalUrl)
  ...
  for(let i = 0; i < result.length;i++){
    ...
    dates.push({
      ...
      firstNameInstructor:instructor.firstName,
      lastNameInstructor:instructor.lastName
    })
  }
...
export default {getCandidates,signNewClass,getAllClassesEndPoint,searchCandidates,timeLeft,deleteClass,getAllClassesUser} 