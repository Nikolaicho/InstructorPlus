...
interface TableForEventsProps {
    title: string;  
    rows: [{event:string,date:string,result:string|undefined,id:string}];
    isPayments:boolean 
}


const TableForEvents: React.FC<TableForEventsProps> = ({ title, rows,isPayments }) => {
    ...
    <div className = "w-[60%] relative left-1/2 transform -translate-x-1/2  ">
        <div className = "title bg-gray-300 border-1 border-black text-center mt-10">{title}</div>
        {localRows.map((rowInfo:any,key:any)=>(
            <div>
                <div className="text-center border-r border-l border-b border-black text-sm py-2 relative">{rowInfo.event} на {rowInfo.date} {rowInfo.result}
                    {!isPayments ? 
                    <div  className="absolute top-[5px] right-[5px] cursor-pointer" onClick={()=>{
                        deleteExam(rowInfo.id)
                        deleteElementFromUI(localRows,key)
                    }} ><DeleteIcon/></div>:<></>}
                    {isPayments ? <div  className="absolute top-[5px] right-[5px] cursor-pointer" onClick={()=>{
